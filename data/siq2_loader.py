...
class SIQ2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if vid_id in self.transcripts_features and torch.rand(1) >= self.skip_transcript_prob:
            transcript_features = self.transcripts_features[vid_id]['features']
            transcript_attention_mask = self.transcripts_features[vid_id]['attention_mask']
        else:
            transcript_features = torch.zeros(1, 1, 768).cuda()
            transcript_attention_mask = torch.zeros(1, 1).cuda()
...
def siq2_collate_fn(batch):
    ...
    transcripts_dim1_max_len = max(batch[i]["transcript_features"].shape[1] for i in range(len(batch)))
    for i in range(len(batch)):
        if batch[i]["transcript_features"].shape[1] < transcripts_dim1_max_len:
            batch[i]["transcript_features"] = torch.cat(
                [
                    batch[i]["transcript_features"],
                    torch.zeros(
                        batch[i]["transcript_features"].shape[0],
                        transcripts_dim1_max_len - batch[i]["transcript_features"].shape[1], 
                        batch[i]["transcript_features"].shape[2]
                    ).cuda(),
                ],
                1,
            )
            batch[i]["transcript_attention_mask"] = torch.cat(
                [
                    batch[i]["transcript_attention_mask"],
                    torch.zeros(
                        batch[i]["transcript_attention_mask"].shape[0],
                        transcripts_dim1_max_len - batch[i]["transcript_attention_mask"].shape[1]
                    ).cuda(),
                ],
                1,
            )
        ...
        if batch[i]["transcript_features"].shape[0] < transcripts_dim0_max_len:
            ...
            batch[i]["transcript_attention_mask"] = torch.cat(
                [
                    batch[i]["transcript_attention_mask"],
                    torch.zeros(
                        transcripts_dim0_max_len - batch[i]["transcript_attention_mask"].shape[0], 
                        transcripts_dim1_max_len
                    ).cuda(),
                ],
                0,
...
def get_siq2_loaders(args, questions_features, attended_questions_features, answers_features, transcripts_features):
    test_dataset = SIQ2Dataset(
    ...
    test_loader = DataLoader(
        ...
        shuffle=False,
        collate_fn=siq2_collate_fn,
    )

    val_dataset = SIQ2Dataset(
